...
def dashboard():
    ...
    top_productos_query = db.session.query(
        Producto.nombre, func.sum(Venta.cantidad).label('total')
    ).join(Venta).filter(Venta.id_vendedor == id_vendedor).group_by(Producto.id).order_by(func.sum(Venta.cantidad).desc()).limit(3).all()
...
def reporte_ventas():
    ...
    for f_code, data in facturas_dict.items():
        ...
        if fecha.day < 5:
            ...
            mes_ciclo = (fecha.replace(day=1) - datetime.timedelta(days=1)).strftime('%B %Y')
        else:
